...
```{r effect_comparison, eval=FALSE, echo=FALSE}
...
In Stan model this AR(1) structure of correlated residuals is calculated as follows
...
Besides correlated residuals, we can also model the correlation between observations

```{r graph_with_gamma_ar1, echo=FALSE, eval=FALSE, message=FALSE, warning=FALSE, cache=TRUE}
initial_graph <- mebn.new_graph_with_randomvariables(datadesc)
sysdimet_gamma_arr1 <- mebn.bipartite_model(reaction_graph = initial_graph, 
                                   inputdata = sysdimet,
                                   predictor_columns = assumedpredictors, 
                                   assumed_targets = assumedtargets, 
                                   group_column = "SUBJECT_ID",
                                   local_estimation = mebn.sampling,
                                   local_model_cache = "models/BLMM_gamma/arr1", 
                                   stan_model_file = "mebn/BLMM_gamma_arr1.stan",
                                   normalize_values = TRUE)

mebn.write_gexf(sysdimet_gamma_ar1, "sysdimet_gamma_arr1.gexf")
write_graph(sysdimet_gamma_ar1, "sysdimet_gamma_arr1.graphml", "graphml")
...
```{r gamma_ar1_loo, message=FALSE, warning=FALSE, cache=TRUE, echo=TRUE}
arr1_vs_ar1 <- mebn.LOO_comparison(temptarget, "BLMM_gamma/arr1", "BLMM_gamma/ar1")
...
# Testing RHS model without holdout

```{r gamma_rhs_arr1_model, echo=TRUE, message=FALSE}
# no holdout
holdout_index <- rep(0,length(sysdimet$SUBJECT_ID))
...
```{r graph_with_gamma_rhs_arr1, echo=FALSE, eval=FALSE, message=FALSE, warning=FALSE, cache=TRUE}
initial_graph <- mebn.new_graph_with_randomvariables(datadesc)
sysdimet_gamma_arr1 <- mebn.bipartite_model(reaction_graph = initial_graph, 
                                   inputdata = sysdimet,
                                   ...
                                   group_column = "SUBJECT_ID",
                                   local_estimation = mebn.sampling,
                                   local_model_cache = "models/BLMM_gamma/rhs_arr1", 
                                   stan_model_file = "mebn/BLMM_gamma_ar1_rhs_pred3.stan",
                                   reg_params = shrinkage_parameters,
                                   normalize_values = TRUE)

write_graph(sysdimet_gamma_arr1, "sysdimet_gamma_arr1.graphml", "graphml")

```

```{r gamma_ar1_ppc, echo=FALSE, message=FALSE, warning=FALSE, eval=TRUE, cache=TRUE}
mebn.target_dens_overlays("BLMM_gamma/rhs_arr1/", assumedtargets, sysdimet)
```
...
typical_effects$strength <- beta$value 
typical_effects$"CI-10%" <- beta$value_lCI
typical_effects$"CI-90%" <- beta$value_uCI

ordered_typical_effects <- typical_effects[order(-typical_effects$strength),]
row.names(ordered_typical_effects) <- NULL
```

```{r typical_effects_table, message=FALSE, warning=FALSE, cache=TRUE, echo=FALSE}
kable(head(ordered_typical_effects, 15), format = "html", caption = "Typically most significant effects of nutrition in Sysdimet study") %>%
  kable_styling(bootstrap_options = c("striped", "condensed"), full_width = FALSE) %>%
  row_spec(0,bold=TRUE)  
```

```{r typical_effects_table_latex, message=FALSE, warning=FALSE, cache=TRUE, echo=FALSE}
#kable(head(ordered_typical_effects, 15), format = "latex", caption = "Typically most significant effects of nutrition in Sysdimet study") %>%
...
row.names(ordered_personal_variance) <- NULL
```

```{r personal_variance_comparison_table, message=FALSE, warning=FALSE, cache=TRUE, echo=FALSE}
kable(head(ordered_personal_variance, 15)) %>%
  kable_styling(bootstrap_options = c("striped", "condensed"), full_width = FALSE) %>%
  row_spec(0,bold=TRUE)  
```

```{r typical_effects_table_latex, message=FALSE, warning=FALSE, cache=TRUE, echo=FALSE}
kable(head(ordered_personal_variance, 15), format = "latex", caption = "Effects with most personal variance in Sysdimet study") %>%
  kable_styling(latex_options = c("striped", "condensed"), full_width = FALSE) %>%
  row_spec(0,bold=TRUE) 
```
...
```{r variation clusters2, echo=FALSE, eval=TRUE, fig.width=8, fig.height=9, cache=TRUE}
source("mebn/MEBN.r")
...
```{r variation clusters3, echo=FALSE, eval=TRUE, fig.width=8, fig.height=8}
source("mebn/MEBN.r")
# Number of cluster centers 
k <- 4
...
```{r variation clusters, echo=FALSE, eval=TRUE, fig.width=8, fig.height=8}
...
holdout_subject_index <- 1

holdout_index <- as.vector(sysdimet$SUBJECT_ID)

holdout_index[holdout_index == "S01"] <- 1
holdout_index[holdout_index != "S01"] 

holdout_index <- c(0,0,0,0,1,1,1,1, rep(0,length(sysdimet$SUBJECT_ID)-8))
#holdout_index <- rep(0,length(sysdimet$SUBJECT_ID))
holdout_index
...
sysdimet_gamma_ar1_rhs_pred <- mebn.bipartite_model(reaction_graph = initial_graph, 
                                   ...
                                   group_column = "SUBJECT_ID",
                                   local_estimation = mebn.sampling,
                                   local_model_cache = paste0("models/BLMM_gamma/ar1_rhs/2"), 
                                   stan_model_file = "mebn/BLMM_gamma_ar1_rhs_pred3.stan",
                                   reg_params = shrinkage_parameters,
                                   normalize_values = TRUE)

write.graph(sysdimet_gamma_ar1_rhs_pred, "subject_02.graphml", "graphml")
```
