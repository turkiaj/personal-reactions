...
- This enables us to look at the joint probability as a whole
  - Principal components as a graph visualization (TIKZ)
...
- Clustering: 
  -- This shows latent intermediate grouping
  - We can explore the personal effects by seeking clusters of similarly reacting patients. Intuition is that not everyone is   reacting individually, but there might be groups of similarly reacting patients that we are not currently aware of.
  
    - Clusters of personal effects verying from average levels
    - Clusters of absolute personal effects
  
    - Can these clusters be matched to genes?
  
    - Note that this grouping is now found by post estimation analysis, but these groups might be found also during the     estimation

- Finally, personal level predictions
...
sysdimet_visgraph <- mebn.visualization_graph(sysdimet_gamma_ar1)
...
top_neg_edges <- head(alledges[order(alledges$weight)], 20)
top_pos_edges <- head(alledges[order(-alledges$weight)], 20)
top_pers_edges <- head(alledges[order(-alledges$b_sigma)], 20)
...
V(sysdimet_visgraph)$size = 20 # MEBN size = 1 !!

bipa <- layout_as_bipartite(sysdimet_visgraph, types = V(sysdimet_visgraph)$type == "200", vgap = 50)

E(sysdimet_visgraph)$color="red"
E(sysdimet_visgraph)[E(sysdimet_visgraph)$weight < 0]$color="blue"

# Idea: make own edge for personal variance to visualization graph

#plot(sysdimet_visgraph, shape="circle", layout=bipa, xlim=c(-10,10), ylim=c(-10,10), rescale=FALSE)
plot(sysdimet_visgraph, shape="circle", layout=bipa, rescale=TRUE)
