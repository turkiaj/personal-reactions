...
Let us explore these identified personal differences in effects of nutrition. Although there are personal differences, it is likely that not everyone behave uniquely but there might exist similar groups of behavior. We can analyze personal differences in two ways. We can look the absolute magnitudes of effects and we can also look how persons differ from typical behavior or mean of the effect.

For this analysis we can collect the estimations of personal effects and the can use k-means algorithm to find similarly behaving clusters within patients. Note that now we are using already estimated effects from the whole dataset. In last part, we show the predictive ability of our model and predict personal graphs.
...
#pe <- summary(fsins_gamma_ar1, pars="personal_effect", probs = c(0.25, 0.75))$summary[,c(1,4,5)]
#pe
