...
assumedtargets <- datadesc[datadesc$Name=="fshdl",] 
...
Log-normal distribution might be easier to sample, if we can get the regression coefficients to absolute and additive scale

```{r, echo=TRUE, eval=TRUE, message=FALSE, warning=FALSE}
source("mebn/MEBN.r")

assumedtargets <- datadesc[datadesc$Name=="fsins",] 
...
sysdimet_graph <- mebn.typical_graph(reaction_graph = initial_graph, 
                                   ...
                                   group_column = "SUBJECT_ID",
                                   local_estimation = mebn.sampling,
                                   local_model_cache = "models/BLMM_lognormal", 
                                   stan_model_file = "mebn/BLMM_lognormal.stan",
                                   edge_significance_test = my.RanefTest, 
                                   normalize_values = TRUE, 
                                   reg_params = shrinkage_parameters)
```
...
## Comparing normal and gamma regressions.. absolute parameters should match?
  ...
  fsins_gamma <- mebn.get_localfit("BLMM_gamma/fsins")
  ...
  posterior_g <- extract(fpgluk_gamma, pars = c("Y_rep", "beta", "beta_Intercept"))
