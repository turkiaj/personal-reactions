...
```{r normal_model_ppc, echo=FALSE, eval=TRUE, message=FALSE}
source("mebn/MEBN.r")
...
```{r gamma_ppc, echo=FALSE, eval=TRUE, message=FALSE}
...
```{r, personal_variations_figure, fig.height = 7, fig.width = 10, fig.align = "center"}
source("mebn/MEBN.r")
...
First, we prepare the 5-fold datasets for training and testing. It is important that all observations for one patient are included either in training or testing. This is taken care of by "kfold_split_stratified" function in LOO package.

```{r 5-fold split}
library(loo)
set.seed(345)
fold5_indexes <- kfold_split_stratified(K = 5, x = sysdimet$SUBJECT_ID)
...
sysdimet_gamma_ar1_rhs <- mebn.bipartite_model(reaction_graph = initial_graph, 
                                   ...
                                   group_column = "SUBJECT_ID",
                                   local_estimation = mebn.sampling,
                                   local_model_cache = paste0("models/BLMM_gamma/ar1_pred/", h), 
...
fit1  <- mebn.get_localfit("BLMM_gamma/ar1_pred/1/fshdl")
...
ggtitle("fshdl - ar1_pred")

fit2  <- mebn.get_localfit("BLMM_gamma/ar1_rhs_pred/1/fshdl")

fshdl.true <- holdout_data$fshdl
pred_post <- extract(fit2, pars = c("Y_pred"))
post_50 <- na.omit(pred_post$Y_pred[seq(2000,4000,10),])

ppc_dens_overlay(fshdl.true, post_50) + 
coord_cartesian(xlim = c(0,5.0)) +
ggtitle("fshdl - ar1_rhs_pred")


fshdl.true <- training_data$fshdl
rep_post <- extract(fit, pars = c("Y_rep"))
post_50 <- na.omit(rep_post$Y_rep[seq(2000,4000,10),])

ppc_dens_overlay(fshdl.true, post_50) + 
coord_cartesian(xlim = c(0,5.0)) +
ggtitle("Veren HDL-kolestrolin ennuste")
