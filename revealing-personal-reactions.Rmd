...
```{r}
source("mebn/MEBN.r")
comp <- mebn.LOO_comparison3(assumedtargets, "BLMM_normal", "BLMM_gamma/hierarchical_idlink", "BLMM_gamma/ar1")
comp <- rbind(comp, c("", sum(comp[,2]), sum(comp[,3]), sum(comp[,4])))

kable(comp) %>%
  kable_styling(bootstrap_options = c("striped", "condensed"), full_width = TRUE) %>%
  row_spec(0,bold=TRUE)

#kable(comp, "latex", booktabs = T) %>%
