...
p <- as.matrix(personal_pred$Y_pred)

ext_pred <- extract(personal_pred)

out_mat <- matrix(NA, nrow = dim(ext_pred$Y_pred)[2], ncol = dim(ext_pred$Y_pred)[3])

for(i in 1:dim(ext_pred$Y_pred)[2]) {
  for(j in 1:dim(ext_pred$Y_pred)[3]) {
      out_mat[i, j] <- mean(ext_pred$Y_pred[, i, j])
  }
}
