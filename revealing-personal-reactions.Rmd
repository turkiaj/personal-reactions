...
# TODO: Create personal graph by adding nodes for personal effects
# We already have nodes for b_sigma-hyperparameters, now we add values for b_mu. Random variables b are normally distributed.
person_id <-9
localfit <- mebn.get_localfit("BLMM_gamma/ar1_rhs_effpred/fshdl")
...
# TODO: tee "localsummaryn" tyyppinen funktio, jolla voi hakea mallista ennusteet

#mebn.personal_effects
...
fshdl_blmm <- mebn.get_localfit("BLMM_gamma/ar1_rhs_effpred/fshdl")
fsldl_blmm <- mebn.get_localfit("BLMM_gamma/ar1_rhs_effpred/fsldl")

s <- mebn.localsummary(fshdl_blmm)
s
posterior <- as.array(fshdl_blmm)
posterior
