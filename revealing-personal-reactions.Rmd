...
<<<<<<< HEAD
This tuens the problem to finding a graph that is most probable given data. To enforce our assumption of biologically plausible graphs, we set the graph prior \(P(G)\) to zero for all other graphs than the previously descibed bipartite graphs with nutrients affecting the bodily responses.
...
Concretely, we use generalized linear mixed-effect models to describe the local probability distributions of Bayesian network. The models have a linear function of nutrients as input and we estimate parameters \(beta\) and \(b\) that optimally give the corresponding response 
=======
...
>>>>>>> 1b38039c65191b7ddecba45b11064604c45dc8a8

# TODO: jatka tästä mallin ja parametrin formulointia..
...
The data we fit to earlier descibed model consists of nutrients and blood test values..
